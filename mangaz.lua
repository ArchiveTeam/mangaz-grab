...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url)
    ...
    elseif string.match(url, "/virgo/view/") then
      ...
      local verkey = json["verkey"]
      local base = json["Location"]["base"]
      local scramble_dir = json["Location"]["scramble_dir"]
      for _, data in pairs(json["Orders"]) do
        check("https://vw.mangaz.com/virgo/view/" .. item_value .. "/i:" .. data["no"])
        check(base .. scramble_dir .. "/" .. data["name"] .. "?" .. verkey)
        check(base .. scramble_dir .. "/" .. data["name"])
      end
    end
    if json then
      extract_json(json)
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["mangaz-9k6chnayiyg5gwac"] = discovered_items,
    ["urls-6c1u76rgrqxbrs6i"] = discovered_outlinks
  }) do
