...
More information about the archiving project can be found on the ArchiveTeam wiki: [マンガ図書館Z](https://wiki.archiveteam.org/index.php?title=マンガ図書館Z)
...
after which the project container can be run:
    ...
    docker run -d --name archiveteam --label=com.centurylinklabs.watchtower.enable=true --log-driver json-file --log-opt max-size=50m --restart=unless-stopped atdr.meo.ws/archiveteam/mangaz-grab --concurrent 1 YOURNICKHERE
...
Have an issue not listed here? Join us on IRC and ask! We can be found at hackint IRC [#mangoes](https://webirc.hackint.org/#irc://irc.hackint.org/#mangoes).
